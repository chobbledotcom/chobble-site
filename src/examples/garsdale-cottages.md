...
I initially started building a Google Calendar integration to handle availability and reservations, but then I discovered [FreeToBook](https://www.freetobook.com/) - a booking system for holiday lets that charges a flat fee of just £1.50 per booking. There are no percentage fees, hidden charges, or opaque pricing models. FreeToBook also offers built-in integrations with Booking.com, Airbnb, Google Hotels, and more - so both properties can still be listed on those platforms, but directly, without a middleman skimming an extra cut.
